...
export function ColorPalette() {
  ...
  const handleSelectPalette = (paletteName: string) => {
    const updatedPalettes = colorPaletteDataState.map((palette) => ({
      ...palette,
      isSelected: palette.name === paletteName,
    }));
    setColorPaletteDataState(updatedPalettes);
    handleClose();
  };
  ...
  const activePalette = colorPaletteDataState.find((p) => p.isSelected);
    ...
    <Box>
      ...
      >
        <Typography color="#161616" fontSize={"14px"}>
          {activePalette ? activePalette.label : "Color Palette"}
        </Typography>
      ...
      >
        {colorPaletteDataState.map((option, i) => {
          return (
            <MenuItem
              key={option.name}
              value={option.label}
              ...
              sx={{
                ...
                fontWeight: 400,
                fontSize: "14px",
                background: option.isSelected ? "#F8F9FA" : "transparent",
                display: "flex",
                alignItems: "center",
                justifyContent: "space-between",
                gap: "8px",
                borderTop: option.isSelected ? "1px solid #E0E0E0" : "none",
                borderBottom: "none",
              }}
            >
              <Button
                sx={{
                  ...
                  borderRadius: "0px",
                  borderBottom:
                    i < colorPaletteDataState.length - 1
                      ? "1px solid #98A1AA"
                      : "none",
                }}
              >
